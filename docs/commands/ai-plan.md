...
2. **Confirms the activated domain.** Step 0 already ran the repo-facts sniff and activated the domain(s); this step reads that result from `activation.md` and confirms it with you. It only asks when stage 1 matched zero domains, or the ask itself is ambiguous or spans platforms/layers. One parent story fans out into **one plan and one sub-issue per domain**. `adlc-design` loads only when the work touches Figma or a design surface.
