...
Every tracker write routes through the four-verb adapter in [references/tracker-adapters.md](references/tracker-adapters.md) (`create_issue`, `link_child`, `set_status`, `assign`); the adapter holds the per-tracker mapping (GitHub, Jira) so the lifecycle prose stays tracker-neutral. Three rules are mandatory on every run:
