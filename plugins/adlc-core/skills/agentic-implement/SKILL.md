...
5. CHECKPOINT, consent, the push gate: before asking, write the checkpoint file (`type: push`, `checkpoints.md`); the user may also resolve it from the cockpit while you wait. Present exactly what would be pushed and STOP for an explicit yes before any `git push` or anything outbound (a deploy, a publish, an API write). There is no "ship" command; this push gate is the only path to a remote, and it never fires without that yes. After the user approves, push, and set `index.md`'s `status: done` (or `abandoned`), the run's terminal state (`checkpoints.md`). The pipeline ends.
