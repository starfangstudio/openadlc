...
6. CHECKPOINT, consent: before asking, write the checkpoint file (`type: post-issue`, `checkpoints.md`). When the user says "done", "create it", or "post it", present exactly what would be posted and STOP for an explicit yes. On GitHub the issue shows `briefing.md` as the body with the AI concepts unwrapped into a collapsible `<details>` (overflow to sequential comments past ~60KB); on Jira/ADO the body is `briefing.md` and the bundle is attached as `<slug>.okf.tgz`. Assign yourself on create and verify the assignee stuck. Nothing is posted without that yes. After posting, record the issue URL back in the bundle, and set `index.md`'s `status: done` if the run ends here (`checkpoints.md`).
