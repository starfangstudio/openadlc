...
# docs-current: fail a PR that changes a command, hook, or config without
...
needs=0; hasdocs=0
echo "$changed" | grep -qE '(/commands/|/hooks/|openadlc\.example\.yaml)' && needs=1
echo "$changed" | grep -qE '(^|/)docs/' && hasdocs=1

if [ "$needs" = 1 ] && [ "$hasdocs" = 0 ]; then
  echo "docs-current: a command, hook, or config changed but docs/ did not."
